...
impl ItemFetcher<'_> {
    ...
    async fn fetch(
        ...
        let (cached_page_len, next_page_token, cached_total): (u32, Option<String>, Option<u32>) =
            redis::pipe()
                .llen(cache_key.clone() + "items")
                .get(cache_key.clone() + "next_page")
                .get(cache_key.clone() + "total")
        ...
        if let Some(cached_total) = cached_total {
            total.store(cached_total, Ordering::Relaxed);
        }
    ...
    async fn fetch_from_yt(
        ...
        let cache_update = async {
            let items_key = cache_key.clone() + "items";
            ...
            pipe.atomic()
                ...
                .set_ex(cache_key.clone() + "total", total_results, expiration);
            if let Some(next_page_token) = next_page_token.as_ref() {
                pipe.set_ex(cache_key.clone() + "next_page", next_page_token, expiration);
            }
...
pub async fn search(
    ...
    let fetched = ItemFetcher {
        ...
        cache,
        client: &client,
        pages_namespace: cache_namespace.clone() + "pages:",
        calls_remaining: AtomicU32::new(youtube_api.max_upstream_requests),
    }
    .fetch(
        cache_namespace + "first-page:",
        page_start,
        &mut items,
        None,
        &total,
