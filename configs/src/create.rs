...
pub async fn create(
    ...
    // Insert all roles
    let (names, max_likes): (Vec<String>, Vec<i32>) = roles
        .iter()
        .map(|(name, QueueUserRoleCreate { max_likes, .. })| (name.clone(), *max_likes as i32))
        .unzip();
    let role_ids: Vec<Uuid> = sqlx::query_scalar!(
        "
        INSERT INTO queue_user_role (config_id, name, max_likes) 
        SELECT $1 as config_id, * FROM UNNEST($2::text[], $3::int[]) AS t(name, max_likes)
        RETURNING id
        ",
        config_id,
        &names,
    ...
    sqlx::query!(
        ...
        SELECT * FROM UNNEST($1::uuid[], $2::queue_user_action[]) AS t(role_id, permission)
        ",
        &role_ids,
        &permissions as &[QueueUserAction]
    )
    .execute(&mut tx)
    .await
    .map_err(SqlError::from)?;
    ...
    sqlx::query!(
        ...
        SELECT * FROM UNNEST($1::uuid[], $2::uuid[]) AS t(role_id, granted_role_id)
        ",
        &role_ids,
        &can_grant
    )
    ...
    sqlx::query!(
        ...
        SELECT * FROM UNNEST($1::uuid[], $2::uuid[]) AS t(role_id, removed_role_id)
        ",
        &role_ids,
        &can_revoke
    )
    ...
    let inserted = sqlx::query!(
        ...
        ",
        config_id,
        (name_mapping[creator_role.as_str()]),
        (name_mapping[default_role.as_str()]),
        (name_mapping[banned_role.as_str()]),
        autolike
    )
    .fetch_one(&mut tx)
    .await
