...
class AuthServiceBrowser implements AuthService {
    ...
    public get userData(): UserData | null { return this._userData }

    private set userData(data: UserData | null) {
        this._userData = data;
        if (this.userData) { localStorage.setItem('apelleUser', JSON.stringify(this.userData)) }
...
class AuthServiceStub implements AuthService {
    public async signin(auth: Credentials): Promise<void> { return Promise.reject(new Error('Auth is not implemented outside the browser')) }
    public async signup(auth: Credentials): Promise<void> { return Promise.reject(new Error('Auth is not implemented outside the browser')) }
    public async signout(): Promise<void> { return Promise.reject(new Error('Auth is not implemented outside the browser')) }
    public authenticated(): boolean { throw new Error('Auth is not implemented outside the browser') }
}

export const authService: AuthService = $state(browser ? new AuthServiceBrowser() : new AuthServiceStub());
